...
class TerminalEmulator {
  ...
  private void doEscSelectLeftParen(byte c) {
    doSelectCharSet(true, c);
  }

  private void doEscSelectRightParen(byte c) {
    doSelectCharSet(false, c);
  }

  private void doSelectCharSet(boolean isG0CharSet, byte c) {
    switch (c) {
  ...
  private void doEscPound(byte c) {
    switch (c) {
  ...
  private void doEscLeftSquareBracket(byte c) {
    switch (c) {
  ...
  private void unimplementedSequence(byte c) {
    if (Terminal.LOG_UNKNOWN_ESCAPE_SEQUENCES) {
  ...
  private void unknownSequence(byte c) {
    if (Terminal.LOG_UNKNOWN_ESCAPE_SEQUENCES) {
      logError("unknown", c);
    }
    finishSequence();
  ...
  private void logError(String errorType, byte c) {
    if (Terminal.LOG_UNKNOWN_ESCAPE_SEQUENCES) {
      StringBuilder buf = new StringBuilder();
