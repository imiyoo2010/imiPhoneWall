...
public interface Screen {
   ...
   * @param c ASCII character to store
   * @param foreColor the foreground color
   * @param backColor the background color
   */
  void set(int x, int y, byte c, int foreColor, int backColor);
   ...
   * within the bounds of the screen, or else an InvalidParemeterException will
   * be thrown.
   *
   * @param sx
   *          source X coordinate
   ...
   * this is called with a "val" argument of 32 to clear a block of characters.
   *
   * @param sx
   *          source X
   * @param sy
